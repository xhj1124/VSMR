...
def get_args_parser():
    ...
    parser.add_argument('--dataset', default='rsvgd', type=str,
                        help='referit/flickr/unc/unc+/gref')
    parser.add_argument('--max_query_len', default=40, type=int,
                        help='maximum time steps (lang length) per batch')
    
    # dataset parameters
    parser.add_argument('--output_dir', default='./outputs/ltvg',
                        help='path where to save, empty for no saving')
    parser.add_argument('--device', default='cuda',
                        help='device to use for training / testing')
    parser.add_argument('--seed', default=13, type=int)
    parser.add_argument('--resume', default='', help='resume from checkpoint')
    parser.add_argument('--detr_model', default='/home/xd/xhj/detr-r50-e632da11.pth', type=str, help='detr model')
    ...
    parser.add_argument('--eval_model', default='/home/dell/xhj/xiugai2/checkpoint0129.pth', type=str)

    return parser


def main(args):
    os.environ['MASTER_PORT'] = '8800'  # 端口号随意指定
    ...
    print('trainset:', len(dataset_test))
    ...
    if args.output_dir and utils.is_main_process():
        with (output_dir / "eval_log_erjiyindao.txt").open("a") as f:
            f.write(str(args) + "\n")
    
    start_time = time.time()

    # perform evaluation
    accuracy,accuracy6,accuracy7,accuracy8,accuracy9,meaniou,cumuIou = evaluate(args, model, data_loader_test, device)
    ...
    if utils.is_main_process():
        ...
        log_stats = {'test_model:': args.eval_model,
                    '%s_set_accuracy'%args.eval_set: accuracy,
                     '%s_set_accuracy6' % args.eval_set: accuracy6,
                     '%s_set_accuracy7' % args.eval_set: accuracy7,
                     '%s_set_accuracy8' % args.eval_set: accuracy8,
                     '%s_set_accuracy9' % args.eval_set: accuracy9,
                     '%s_set_meaniou' % args.eval_set: meaniou,
                     '%s_set_cumuIou' % args.eval_set: cumuIou,
                    }
        print(log_stats)
       # if args.output_dir and utils.is_main_process():
                #with (output_dir / "eval_log_erjiyindao .txt").open("a") as f:
