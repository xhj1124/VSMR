...
from .attention import spitial_attention,channels_attention
...
class DETR(nn.Module):
    ...
    def forward(self, samples,text):
        ...
        list.append(samples)
        list.append(text)
        src,pos = self.backbone(list)
        src,mask = src[-1].decompose()
        assert mask is not None
        if self.transformer is not None:
            out = self.transformer(self.input_proj(src), mask, pos[-1], query_embed=None)
            #out = self.transformer(self.input_proj(src), mask, pos[-1], query_embed=None)
        else:
            out = [mask.flatten(1), self.input_proj(src).flatten(2).permute(2, 0, 1)]
