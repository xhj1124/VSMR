...
def get_fashion_mnist_labels(labels):
    ...
    text_labels = ['tenniscourt', 'Expressway-Service-area', 'bridge', 'groundtrackfield', 'overpass',
                   'windmill', 'ship', 'Expressway-toll-station', 'airport', 'vehicle', 'dam', 'harbor',
                   'airplane', 'stadium', 'trainstation', 'storagetank', 'baseballfield', 'golffield',
                   'chimney', 'basketballcourt']
    return [text_labels[int(i)] for i in labels]
def get_labels_index(label):
    """返回Fashion-MNIST数据集的文本标签"""
    text_labels = ['tenniscourt', 'Expressway-Service-area', 'bridge', 'groundtrackfield', 'overpass',
                   'windmill', 'ship', 'Expressway-toll-station', 'airport', 'vehicle', 'dam', 'harbor',
                   'airplane', 'stadium', 'trainstation', 'storagetank', 'baseballfield', 'golffield',
...
class TransVGDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit', 
        ...
        self.tokenizer = BertTokenizer.from_pretrained('/home/dell/xhj/TransVG_self/betr/')
        ...
        if split == 'train':
            ...
            self.augment = False
        if self.dataset != 'rsvgd'and self.dataset!='hgd_rsvgd'and self.dataset!='self_data':
            ...
            file = open('/home/dell/xhj/DataSet/RSVG/' + split  + '.txt', "r").readlines()
            print('/home/xdu/xhj/RSVG-pytorch-main/RSVGD/' + split  + '.txt')
            print('/home/xdu/xhj/RSVG-pytorch-main/RSVGD/Annotations_new')
            print('/media/xdu/zxh/Mymodel/DIOR_RSVG/JPEGImages')
            Index = [int(index.strip('\n')) for index in file]
            count = 0
            annotations = filelist('/home/dell/xhj/DataSet/RSVG/Annotations_new', '.xml')
            for anno_path in annotations:
                ...
                for member in root.findall('object'):
                    if count in Index:
                        ...
                        imageFile = str('/home/dell/xhj/DataSet/RSVG/JPEGImages') + '/' + root.find("./filename").text
            ...
            file = open('/home/dell/xhj/DataSet/RSVG/split/' + split + '.txt', "r").readlines()
            Index = [int(index.strip('\n')) for index in file]
            count = 0
            annotations = filelist('/home/dell/xhj/DataSet/RSVG/Annotations', '.xml')
            for anno_path in annotations:
                root = ET.parse(anno_path).getroot()
                for member in root.findall('object'):
                    if count in Index:
                            ...
                            imageFile = str('/home/dell/xhj/DataSet/RSVG/Image') + '/' + root.find(
                                "./filename").text
    ...
    def pull_item(self, idx):
        if self.dataset != 'rsvgd' and self.dataset!='self_data':
